...
void MeshRenderer::setup(const VulkanDevice* const device, Mesh* meshRef) {
	...
	this->vertexBuffer = GPUBuffer::create(device, meshRef->vertexCount * sizeof(Vertex), vertexFlags, memProps, meshRef->vertexData);
	
	VkBufferUsageFlags indexFlags = VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
	this->indexBuffer = GPUBuffer::create(device, meshRef->indexCount * sizeof(uint32_t), indexFlags, memProps, meshRef->indexData);
}
