...
struct GPUBuffer {
	...
	static GPUBuffer* create(const VulkanDevice* const device, VkCommandPool_T* commandPool, uint32_t sizeInBytes,
		VkBufferUsageFlags usage, const void* data = nullptr);
	static void copyBuffer(const VulkanDevice* const device, VkCommandPool_T* commandPool, VkBuffer_T* src, VkBuffer_T* dst,
		VkDeviceSize size, uint32_t dstOffset = 0);
	...
	void bufferData(const VulkanDevice* const device, VkCommandPool_T* commandPool, const void* data, uint32_t sizeInBytes, uint32_t offset = 0);
