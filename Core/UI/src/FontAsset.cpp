...
FontAsset* const FontAsset::create(const VulkanInstance* const instance, const char* filePath, const float pointSize) {
	...
	stbtt_PackFontRange(&packContext, fontBuffer, 0, font->fontSize, DEFAULT_START_CHARACTER, font->numGlyphs, font->packedChars);
	...
	uint32_t sizeInBytes = sizeof(uint8_t) * font->atlasSize * font->atlasSize;
	font->texture = GPUTexture::createTextureLoadImmediate(instance, font->atlasSize, font->atlasSize, 1, sizeInBytes, font->fontAtlasTextureData, "default-font");
