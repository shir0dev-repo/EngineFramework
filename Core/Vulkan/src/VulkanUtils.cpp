...
void createVkImage(const VulkanContext* const instance, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
    ...
    VkDevice_T* logicalDevice   = instance->getDevice()->getLogicalDevice();
    VkPhysicalDevice_T* physDev = instance->getDevice()->getPhysicalDevice();
    ...
    allocInfo.memoryTypeIndex = getMemoryType(physDev, memRequirements.memoryTypeBits, properties);
...
void createVkImageView(const VulkanContext* const instance, VkImage_T* image, VkFormat format, VkImageAspectFlags aspectMask, VkImageView_T*& view) {
    ...
    if (vkCreateImageView(instance->getDevice()->getLogicalDevice(), &viewInfo, nullptr, &view) != VK_SUCCESS) {
